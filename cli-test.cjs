...
var require_axios = __commonJS({
  "node_modules/axios/dist/node/axios.cjs"(exports2, module2) {
    ...
    var trackStream = (stream2, chunkSize, onProgress2, onFinish) => {
      ...
      return async (config) => {
        ...
        try {
          if (onUploadProgress && supportsRequestStream && method !== "get" && method !== "head" && (requestContentLength = await resolveBodyLength(headers, data)) !== 0) {
            ...
            if (_request.body) {
              const [onProgress2, flush] = progressEventDecorator(
                requestContentLength,
                progressEventReducer(asyncDecorator(onUploadProgress))
              );
              data = trackStream(_request.body, DEFAULT_CHUNK_SIZE, onProgress2, flush);
            }
          }
          ...
          if (supportsResponseStream && (onDownloadProgress || isStreamResponse && unsubscribe)) {
            ...
            const [onProgress2, flush] = onDownloadProgress && progressEventDecorator(
              responseContentLength,
              progressEventReducer(asyncDecorator(onDownloadProgress), true)
            ) || [];
            response = new Response(
              trackStream(response.body, DEFAULT_CHUNK_SIZE, onProgress2, () => {
                flush && flush();
...
// src/core/collectors/git.ts
async function collectGitCommits(config, date) {
  ...
  try {
    ...
    while (hasMore) {
      ...
      for (const match of result.messages.matches) {
        ...
        const ts = match.ts || "";
        const permalink = match.permalink || void 0;
        if (ts) {
          const timestamp = new Date(parseFloat(ts) * 1e3);
          messages.push({
            text,
            channel: channelId,
            channelName,
...
var SYSTEM_PROMPT_KO = `\uB2F9\uC2E0\uC740 \uC5C5\uBB34\uC77C\uC9C0 \uC791\uC131\uC744 \uB3C4\uC640\uC8FC\uB294 AI \uC5B4\uC2DC\uC2A4\uD134\uD2B8\uC785\uB2C8\uB2E4.
...
- \uCD5C\uB300 3-5\uC904`;
var SYSTEM_PROMPT_EN = `You are an AI assistant helping to write work logs.
...
- Maximum 3-5 lines`;
function buildUserPrompt(period, commits, messages, language) {
  const periodLabel = language === "ko" ? period === "am" ? "\uC624\uC804 (09:00-12:00)" : "\uC624\uD6C4 (13:00-18:00)" : period === "am" ? "Morning (09:00-12:00)" : "Afternoon (13:00-18:00)";
  let prompt = language === "ko" ? `${periodLabel} \uC5C5\uBB34\uB0B4\uC6A9\uC744 \uC694\uC57D\uD574\uC8FC\uC138\uC694.
...
`;
  ...
  if (commits.length === 0 && messages.length === 0) {
    prompt += language === "ko" ? "(\uC774 \uC2DC\uAC04\uB300\uC5D0 \uAE30\uB85D\uB41C \uD65C\uB3D9\uC774 \uC5C6\uC2B5\uB2C8\uB2E4)" : "(No recorded activity for this period)";
  }
  return prompt;
}
...
async function summarizeDay(config, data) {
...
async function ensureAuthenticated(config, generalConfig, authStatePath) {
  if ((0, import_fs2.existsSync)(authStatePath)) {
...
async function collectWeekData(weekStart, config, onProgress2, signal) {
...
async function generateSummary(data, config, onProgress2, signal) {
  const logs = [];
  function log(level, message) {
    logs.push({ timestamp: /* @__PURE__ */ new Date(), level, message });
    onProgress2?.({
...
async function executeAutomation(summary, config, onProgress2, signal) {
...
async function executeFullPipeline(weekStart, config, onProgress2, signal) {
  const weekData = await collectWeekData(weekStart, config, onProgress2, signal);
  const summary = await generateSummary(weekData, config, onProgress2, signal);
  await executeAutomation(summary, config, onProgress2, signal);
  return summary;
}
